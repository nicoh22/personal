...
typedef struct perro_t
{
	...
	// ~~~ para ser completado ~~~
	uint index;  // indice de 0 a 15     0
	struct jugador_t *jugador;//8

	uint id;     // id unica tarea		16
	uint tipo;   // raza del perro		24
	uint libre;  // libre: -true- slot disponible para lanzar perro  / -false- ya hay un perro vivo	32
...
typedef struct jugador_t
{
	...
	direccion orden; //inicializar en AQUI
	int x, y;  // posicion
	uint puntos;
...
float game_perro_recibirOrden(perro_t *perro);
