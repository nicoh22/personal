...
\Rep[rtr][r]{noCiclos(r.raiz, $\emptyset$) \yluego esRestriccion?(r.raiz)}

\TipoFuncion{esRestriccion?}{\In{n}{Nodo}}{bool}\\
\indent if n.tipo \igobs CAR \\
\indent \indent then n.izq \igobs NULL $\land$ n.der \igobs NULL\\
\indent \indent else n.izq $\neq$ NULL $\land$ n.der $\neq$ NULL \yluego \\
\indent \indent \indent esRestriccion?(n.izq) $\land$ esRestriccion?(n.der)\\
\indent end if\\
\end{Representacion}

\begin{Algoritmos}\\
\TipoFuncion{< \argumento >}{\In{t}{tag}}{restriccion}\\ 
...
\\
\TipoFuncion{NOT \argumento }{\Inout{r1}{restriccion}}{}\\  
\indent r1.negado? $\leftarrow$ true\\
\\
\TipoFuncion{\argumento AND \argumento }{\In{r1}{restriccion}, \In{r2}{restriccion}}{restriccion}\\
 \indent puntero(Nodo) n $\leftarrow$ New Nodo\\
 ...
 \indent res.raiz $\leftarrow$ n\\
\\ 
\TipoFuncion{\argumento OR \argumento }{\In{r1}{restriccion}, \In{r2}{restriccion}}{restriccion}\\
 \indent puntero(Nodo) n $\leftarrow$ New Nodo\\
 ...
 \indent res.raiz $\leftarrow$ n\\
\\
\TipoFuncion{verifica}{\In{r}{restriccion}, \In{ts}{tags}}{bool}\\
\indent res $\leftarrow$ verificaAux(r.raiz, ts)
\\
\\
\TipoFuncion{verificaAux}{\In{n}{Nodo}, \In{ts}{tags}}{bool}\\
