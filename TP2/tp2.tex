...
actualizada con la posición siguiente. Este iterador se indefine cada vez que el vector se redefine. En ese caso deberá crearse un nuevo iterador.
 ...
 \tadOperacion{convertir}{conj(nat)/cn}{s : secu(nat)}{}
  \tadAxioma{convertir($cn$)}{if $\#cn = 0$ then \\ 
  $\emptyset$ else \\
  dameUno(cn)\argumento convertir(sinUno($cn$)) \\
  fi}
...
\begin{Algoritmos}
  ...
  \Titulo{Algoritmos del diccionario arreglo}
  ...
  \TipoFuncion{iSiguiente}{\In{it}{itTupla}}{nat} \\
  \indent\indent res $\leftarrow$ it.posicion; \\

  \TipoFuncion{iHaySiguiente?}{\In{it}{itTupla}}{bool} \\
